...
import { composerCaret, composerOf, composerText, setComposerCaret } from '../src/client/composer-dom.ts'
...
interface Fixture {
  ...
  /** Select the [start, end) range of the composer text (a non-collapsed selection). */
...
/**
 * The real host composer shape since harness 0.1.2-alpha.5 / 0.1.2-rc.1:
 * `[data-input-scroll]` > contenteditable `div[data-composer-input]`. The
 * identity face is the real {@link composerOf}; only the services stay
 * faked (input-machine state, sessions, triggers).
 */
function fixture(overrides: Partial<ComposerHistoryHost> = {}, config: ComposerHistoryConfig = CONFIG): Fixture {
  ...
  window.addEventListener('input', handle.input)
  cleanups.push(() => {
    window.removeEventListener('keydown', handle.keydown, true)
    window.removeEventListener('input', handle.input)
    handle.dispose()
  })
  const witness = vi.fn()
  ...
  // Mimics the composer wiring: the edit lands in the contenteditable DOM
  // immediately and in the input-machine draft via the bubble-phase onChange.
  ...
  const selectRange = (start: number, end: number): void => {
    ...
    if (selection === null || first === null) return
...
describe('takeover discipline', () => {
  it('prevents default and stops propagation only on takeover, then fills and moves the caret', async () => {
    const fx = fixture()
    setComposerCaret(fx.composer, 0)
    ...
    expect(composerCaret(fx.composer)).toBe('two'.length)
  })

  it('hold at the oldest entry intercepts without any mutation', () => {
    const fx = fixture()
    setComposerCaret(fx.composer, 0)
  ...
  it('ctrl alias off passes ctrl+up; on it takes over', () => {
    ...
    setComposerCaret(off.composer, 0)
    ...
    const on = fixture()
    setComposerCaret(on.composer, 0)
...
describe('browse cycle', () => {
  it('recalls, walks, and restores the stashed draft and caret exactly', async () => {
    ...
    setComposerCaret(fx.composer, 5)
    ...
    await fx.flushFrames()
    expect(composerText(fx.composer)).toBe('hello')
    expect(composerCaret(fx.composer)).toBe(5)
  })

  it('restores an empty draft to empty', () => {
    const fx = fixture()
    setComposerCaret(fx.composer, 0)
    fx.dispatchKey('ArrowUp')
    fx.dispatchKey('ArrowDown')
    expect(fx.setDraftCalls).toEqual(['two', ''])
    expect(composerText(fx.composer)).toBe('')
    expect(fx.handle.state()).toEqual({ kind: 'idle' })
  })
  ...
  it('multiline: middle-line arrows only move the caret, edges recall', () => {
    ...
    setComposerCaret(fx.composer, 3)
...
describe('compaction summaries in history', () => {
  ...
  it('admits compaction summary views into recall when enabled', async () => {
    ...
    expect(fx.handle.state()).toMatchObject({ kind: 'browsing', index: 2 })
    await fx.flushFrames()
    expect(composerText(fx.composer)).toBe('[compacted] earlier work')
  })
...
describe('fill and dispose', () => {
  it('fill writes the text and moves the caret to its end', async () => {
    ...
    expect(fx.handle.state()).toEqual({ kind: 'idle' })
    await fx.flushFrames()
    expect(composerCaret(fx.composer)).toBe('picked'.length)
  })

  it('dispose cancels a pending caret frame', async () => {
    const fx = fixture()
    const doc = fx.composer.ownerDocument
    const spy = vi.spyOn(doc, 'getSelection')
    fx.handle.fill(fx.composer, 'later') // schedules a caret frame for the fill
    fx.handle.dispose()
    spy.mockClear()
    fx.composer.textContent = 'changed-before-frame'
    await fx.flushFrames()
